...
export const Sales: React.FC = () => {
  ...
  const handleEditSale = (sale: Sale) => {
    ...
    const hydratedItems = sale.items.map(item => ({
      ...item,
      internalId: generateLocalId(), // Assign new local IDs for React
    ...
    <div className="space-y-4 font-sans text-sm pb-10">
      ...
      <div className="flex border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 rounded-t-xl no-print transition-colors">
        {(isSalesperson || isCashierConfirming) && (
          <button
            onClick={() => { if(isSalesperson) setActiveTab('new'); }}
        ...
        {isCashier && (
          <button
            onClick={() => { setActiveTab('pending'); resetSalesState(); }}
        ...
        <button
          onClick={() => { setActiveTab('history'); resetSalesState(); }}
      ...
      {activeTab === 'new' && (isSalesperson || isCashierConfirming) && (
        <div className="bg-gray-100 dark:bg-slate-900 flex flex-col gap-4 max-w-[1400px] mx-auto mt-4 transition-colors">
              ...
              <div className="bg-white dark:bg-slate-800 rounded shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden min-h-[200px]">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                    {cart.map((item, idx) => (
                      <tr key={item.internalId || idx} className="hover:bg-blue-50 dark:hover:bg-slate-700/50 group transition-colors">
                  ...
                  <div>
                   ...
                   <div>
                    ...
                    <input 
                      ...
                      disabled={isCashierConfirming}
                      className="w-full border border-gray-300 dark:border-slate-600 rounded px-2 py-1 text-sm outline-none text-right bg-white dark:bg-slate-700 dark:text-white disabled:bg-gray-50 dark:disabled:bg-slate-800" 
                    />
                  </div>
                  <div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-100 dark:bg-slate-900 p-4 rounded shadow-inner border border-gray-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {/* Payment Inputs... */}
                  {/* (Keeping existing payment inputs, just wrapping in correct grid) */}
                  <div>
                ...
                {sales.map(sale => (
                  <tr key={sale.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-xs font-mono text-gray-500 dark:text-gray-400">#{sale.id.slice(0, 8)}</div>
                      <div className="text-sm text-gray-900 dark:text-white">{new Date(sale.createdAt).toLocaleDateString()} {new Date(sale.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                    </td>
      ...
      {/* --- MODALS (Edit, Discount, Cancel, View) --- */}
      {/* Kept largely the same, just ensured z-indexes are correct and keys are unique where applicable */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
           <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md overflow-hidden border dark:border-slate-700">
               ...
               <div className="p-6 space-y-4">
                  ...
                  <button onClick={saveEditItem} className="w-full bg-amber-400 hover:bg-amber-500 dark:bg-amber-600 dark:hover:bg-amber-500 text-slate-900 dark:text-white font-bold py-3 rounded uppercase text-sm">
                    Confirmar
                  </button>
               </div>
           </div>
           ...
           <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md overflow-hidden border dark:border-slate-700">
              ...
              <div className="p-6">
                 ...
                 <div className="flex justify-between mb-6 text-sm">
                    ...
                    <div className="space-y-3 w-32">
                       <div>
                         ...
                         <input type="text" className="w-full border border-gray-300 dark:border-slate-600 rounded p-1 text-right text-sm bg-white dark:bg-slate-700 dark:text-white" value={formatMoney(tempDiscountValue)} onChange={e => handleDiscountValueChange(parseMoney(e.target.value))} />
                       </div>
                       <div>
                         <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 mb-1">→ % TOTAL (Item+Global)</label>
                         <input type="number" className="w-full border border-gray-300 dark:border-slate-600 rounded p-1 text-right text-sm bg-white dark:bg-slate-700 dark:text-white" value={tempDiscountPercent.toFixed(2)} onChange={e => handleDiscountPercentChange(parseFloat(e.target.value)||0)} />
                       </div>
                       <div>
                         <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 mb-1">→ TOTAL C/ DESC R$</label>
                         <input type="text" className="w-full border border-gray-300 dark:border-slate-600 rounded p-1 text-right text-sm font-bold text-blue-600 dark:text-blue-400 bg-white dark:bg-slate-700" value={formatMoney(subTotal - tempDiscountValue)} onChange={e => handleDiscountTotalChange(parseMoney(e.target.value))} />
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="space-y-2">
                    <button onClick={confirmDiscount} className="w-full bg-amber-400 hover:bg-amber-500 dark:bg-amber-600 dark:hover:bg-amber-500 text-slate-900 dark:text-white font-bold py-2 rounded uppercase text-sm flex items-center justify-center">
                      <Check size={16} className="mr-2" /> Confirmar Desconto
                    </button>
                    <button onClick={() => setDiscountModalOpen(false)} className="w-full bg-slate-500 hover:bg-slate-600 dark:bg-slate-600 dark:hover:bg-slate-500 text-white font-bold py-2 rounded uppercase text-sm">
                      Cancelar
                    </button>
                 </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md overflow-hidden border dark:border-slate-700 animate-in zoom-in duration-200">
            <div className="p-6 text-center">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => setCancelModalOpen(false)} className="bg-gray-200 dark:bg-slate-700 text-gray-800 dark:text-white font-bold py-3 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-600 transition-colors">
                  Voltar
                </button>
                <button onClick={executeCancelSale} className="bg-red-600 text-white font-bold py-3 rounded-lg hover:bg-red-700 transition-colors shadow-md">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-2xl overflow-hidden border dark:border-slate-700 flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 bg-gray-50 dark:bg-slate-900 border-t border-gray-200 dark:border-slate-700 flex justify-end">
               <button onClick={() => setViewModalOpen(false)} className="bg-gray-200 hover:bg-gray-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-gray-800 dark:text-white font-bold py-2 px-6 rounded transition-colors">
                 Fechar
               </button>
            </div>
          </div>
